...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('crystalMenu'));
		...
		logoBl.screenCenter(); //Centers the logo
		logoBl.y -= 50;

		logoBl.antialiasing = ClientPrefs.globalAntialiasing;
		logoBl.animation.addByPrefix('bump', 'LogoBump', 24, true); //CRYSTAL CAVERN ENCOUNTER LOGO
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('TeamTopazLogo')); //ngSpr = Team Topaz Logo
		add(ngSpr);
		ngSpr.visible = false;
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 8:
					...
					addMoreText('FNF OC', 15);
					addMoreText('ModJam', 15);
					addMoreText('2023', 15);
					
					//addMoreText('Team Topaz', -40);
					//ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 9:
					//deleteCoolText();
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Encounter'); // credTextShit.text += '\nFunkin';
