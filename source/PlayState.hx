...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		iconP2 = new HealthIcon(dad.healthIcon, true);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.reloadNote('',gottaHitNote ? 'NOTE_assets_player' : 'NOTE_assets_opponent');
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						sustainNote.reloadNote('',gottaHitNote ? 'NOTE_assets_player' : 'NOTE_assets_opponent');
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player == 1)
			{
				babyArrow.set_texture('NOTE_assets_player');
			...
			else
			{
				...
				babyArrow.set_texture('NOTE_assets_opponent');
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			else
			{
				...
				MusicBeatState.switchState(new MainMenuState()); //Note: change this to be freeplay when updating the mod! This is just temporary!
				FlxG.sound.playMusic(Paths.music('crystalMenu'));
				changedDifficulty = false;
			}
			transitioning = true;
	...
	public function spawnNoteSplashOnNote(note:Note, ?isDad:Bool = false) {
		if(ClientPrefs.noteSplashes && note != null) {
			...
			if(strum != null) {
				spawnNoteSplash(strum.x, strum.y, note.noteData, note, isDad);
			}
		}
	}
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null, ?isDad:Bool) {
		...
		var skin:String = 'noteSplashes_topaz';
		...
		if (isDad)
			skin = 'noteSplashes_mirror';

		var splash:NoteSplash = grpNoteSplashes.recycle(NoteSplash);
		splash.setupNoteSplash(x - 90, y - 90, data, skin, hue, sat, brt);
		grpNoteSplashes.add(splash);
	}
